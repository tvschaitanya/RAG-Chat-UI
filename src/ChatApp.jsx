...
export default function ChatApp() {
  ...
  return (
    <div className="flex flex-col h-screen max-w-md mx-auto bg-gray-50 font-sans shadow-xl sm:border-x sm:border-gray-200 relative">
      <header className="bg-white/80 backdrop-blur-md border-b border-gray-200 p-4 sticky top-0 z-10 flex items-center justify-center">
              ...
              <div className="mt-1 flex flex-wrap gap-1 max-w-[85%] pl-1">
                {msg.sources.map((source, idx) => (
                  <a 
                    ...
                    className="text-[10px] text-gray-500 hover:text-blue-500 bg-gray-100 hover:bg-blue-50 border border-gray-200 rounded px-1.5 py-0.5 truncate max-w-[150px] block transition-colors"
                  >
      ...
      <footer className="bg-white border-t border-gray-200 p-3 safe-area-pb">
        <form onSubmit={sendMessage} className="flex gap-2 items-center">
          <input
            ...
            className="flex-1 bg-white border border-gray-300 rounded-full pl-4 pr-10 py-2 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-colors text-[15px]"
            disabled={isLoading}
          />
          <button
            type="submit"
            disabled={!input.trim() || isLoading}
            className="bg-blue-500 text-white rounded-full w-8 h-8 flex items-center justify-center flex-shrink-0 disabled:opacity-50 disabled:bg-gray-300 transition-colors shadow-sm"
          >
